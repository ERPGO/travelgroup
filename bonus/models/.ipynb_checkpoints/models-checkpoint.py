...
class AccountAnalyticInherit(models.Model):
    ...
    is_bonus_eligible = fields.Boolean(string="Bonus Eligible", related="employee_id.is_bonus_eligible")
...
class HRPayslipEval(models.Model):
    ...
    def _get_total_split( self ):
        if self.kpi_split > 0.0:
            self.total_split = self.experience_split + self.kpi_split + self.ot_split
        else:
            return 0.0
